...
export default function LotQrThumbnail({ publicId, batchCode }: { publicId: string; batchCode: string }) {
  const [open, setOpen] = useState(false);
  const src = `/api/qr/lot/${publicId}?format=png`;
  return <>
    ...
    {open && <div className="lot-qr-modal-backdrop" onClick={() => setOpen(false)}>
      <div className="lot-qr-modal" onClick={event => event.stopPropagation()}>
        ...
        <div className="lot-qr-modal-actions">
          <a className="lot-qr-print-link" href={`/api/qr/lot/${publicId}?format=png&download=1`} download>⬇ Tải xuống</a>
          <a className="lot-qr-print-link" href={`/lot/${publicId}/print`} target="_blank" rel="noreferrer">In mã QR</a>
        </div>
      </div>
    </div>}
  </>;
