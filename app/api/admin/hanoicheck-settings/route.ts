...
import { canEncryptHanoiCheckSecret, encryptHanoiCheckSecret, getHanoiCheckStatus } from '@/lib/hanoicheck-settings';
...
export async function PUT(request: Request) {
  ...
  const baseUrl = trimmed(body.baseUrl, 200);
  ...
  if (baseUrl) {
    ...
    catch { return NextResponse.json({ error: 'Endpoint phải là URL https hợp lệ, không kèm /api.' }, { status: 400 }); }
  }
  ...
  await prisma.$transaction(async tx => {
    await tx.hanoiCheckIntegrationSettings.upsert({
      where: { id: 1 },
      create: { id: 1, ...data },
      update: { ...data, ...(clientId || clientSecret ? { encryptedAccessToken: null, encryptedRefreshToken: null, accessTokenExpiresAt: null } : {}) },
    });
